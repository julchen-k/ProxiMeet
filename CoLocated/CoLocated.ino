...
//specification of Firebase credentials
...
bool currentTouch = false; //pushed to Firebase, indicating whether the person is touching at the current moment
bool sentTouch = false; //pushed to Firebase, indicating whether the person has sent a touch
bool user1_sentTouch = false; //pulling from Firebase, indicating whether another user sent a touch
bool user1_currentTouch = false; //pulling from Firebase, indicating whether another user is touching at the current moment

//setup of a timer to check if a certain time span has passed without a mutual touch
...
void loop() {
  ...
  user1_sentTouch = Firebase.getBool("user1_sentTouch"); //checks if the other device was touched
  user1_currentTouch = Firebase.getBool("user1_currentTouch"); //checks if the other device is currently touched
  ...
  //{
    if (touched) // & (1 << i)) 
    {
      ...
      currentTouch = true;
      sentTouch = true;
      Firebase.setBool ("user2_currentTouch",true);
      Firebase.setBool ("user2_sentTouch",true);
      Serial.println("user 2 touching");
      //delay(10);
       
    }
    
    //only this user has sent a touch
    if (sentTouch == true & user1_sentTouch == false)
    {
      blueLightState();
    }
    ...
    if (sentTouch == true & user1_sentTouch == true)
    {
      yellowLightState();
    ...
    if(currentTouch & user1_currentTouch)
    {
      mutualTouch();
...
void noCurrentTouch()
{
  ...
  //delay(10);
	...
	if(user1_sentTouch == true & !sentTouch)
	{
		blueLightState();
    //yield();
    //delay(10);
